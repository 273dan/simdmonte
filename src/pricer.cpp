...
float Pricer::price(const Option& option, const MarketData& market) const {
  ...
  for (int i = 0; i < n_threads; i++) {
    ...
    threads.emplace_back([=, &option, &market, &payoff_sums] {
      std::unique_ptr<IUnderlying> underlying =
          UnderlyingFactory::create(option, market, params_);
      std::unique_ptr<IAccumulator> accumulator =
          option.get_accumulator(params_);
  ...
  float total_payoff_sum =
      std::accumulate(payoff_sums.begin(), payoff_sums.end(), 0.0f);
