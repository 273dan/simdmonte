...
__m256 Rng::uniform() const {
  ...
  /* This function performs some bit manipulation to turn a vector of 4 64-bit ints
   * into a vector of 8 32-bit floats
   *
   * 1. Reinterpret the vector as 8 32-bit
   * 2. Shift each int right by 8 bits to make room for the exponent we will insert
   * 3. Mask in the exponent 0x3F800000, which will make the resulting float lie between 1 and 2
   * 4. Cast the whole vector to float 32 and subtract 1 so the floats lie between 0 and 1
...
__m256 Rng::box_muller_transform() const {
   ...
   *
   * This method uses two calls to random to produce two vectors of normal variables
   * so one is cached for the next call
   *
   */
...
__m256 Rng::inverse_cdf_approx() const {
   ...
   *
   * 2 separate polynomial approximations are used, one for the center and one for each tail
   * of the distribution
   *
   * The constants can be found in rng_constants.h
   *
   *
   * This is currently slower than the Box-Muller transform, but it remains for potential
   * future optimisation
   *
   */
